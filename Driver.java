...
public class Driver {
	...
	void addEmployee(String employeeId){
		...
		if(directory.addNewEmployee(employeeId))
			System.out.println("Employee added  "+"(Look for console messages.)" + line+CONTINUE);
		else
			System.out.println("Employee can not added "+"(Look for console messages.)" + line+CONTINUE);
	}
	void addBook(String bookId){
		//done
		if(directory.addNewBook(bookId))
			System.out.println("Book added " + "(Look for console messages.)"+line+CONTINUE);
		else
			System.out.println("Book can not added "+ "(Look for console messages.)" + line+CONTINUE);
	}
	void checkIfBookIsInStock(String bookId){
		...
		if(directory.bookIsInStock(bookId))
			...
			System.out.println("No, book " + bookId + "is not in stock"+ "(Look for console messages.)"+ line+CONTINUE);
	}
	void checkWhetherFoodItemInMarket(String foodItem){
		...
		if(directory.marketCarriesFoodItem(foodItem))
			...
			System.out.println("No, the market do not carries "+foodItem+line+CONTINUE);
	}
